...
def get_blockchain(period, count, iterations):
    binSize='1m'
    if period == 60:
        binSize='1m'
    elif period == 3600:
    ...
    data=[]   
    url = 'https://www.bitmex.com/api/v1/trade/bucketed?binSize=' + binSize + '&partial=false&symbol=XBTUSD'
...
def readData(instrument_id, path_datasets, symbol, period=60, count=500, iterations=1):
    ...
    def documents2():
        ...
        js=get_blockchain(period, count, iterations)
        for line in js:    
                i=0
                i+=1
                row=dict()
...
def    main():
        #get_blockchain(60, 60)
        inst_id=getInitialData(50)
        while 1:
            try:
                print ('Getting Latest Data History')
                getLatestData(inst_id)
                time.sleep(60)
            except Exception as e:
                print (e)
def    getInitialData(iterations):
        ...
        inst=getInstrument('XBTUSD', 'XBTUSD', 'Blockchain', 'Blockchain', 'XBTUSD')
        btc_usd = readData(inst.id, path_datasets, 'XBTUSD', 60, 500, iterations)
        return inst.id

def    getLatestData(inst_id):
        path_datasets='./data/'
        btc_usd = readData(inst_id, path_datasets, 'XBTUSD', 60, 5, 1)
