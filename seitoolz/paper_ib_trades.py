...
def update_ib_trades(systemname, account, pos, tradepl, purepl, buypower, ibexch, date):
    ...
    pos['qty_pnl']=account['qty_pnl']
    pos['trade_qty_pnl']=account['trade_qty_pnl']
    pos['real_qty_pnl']=account['real_qty_pnl']
    pos['real_trade_qty_pnl']=account['real_trade_qty_pnl']
    if debug2:
    ...
    dataSet['permid'] = dataSet['permid'].astype('int')
    dataSet=dataSet.set_index('permid')   
    dataSet['date']=[parse(str(x)).strftime('%Y-%m-%d %H:%M:%S') for x in dataSet['times']]
    with lock:
        dataSet.to_csv(filename)
