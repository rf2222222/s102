...
def smoothHurst(p,bars,threshold=0.5, showPlot=False):
    index = p.index
    ...
    #to return
    modes = np.where(SmoothHurst<threshold,0,1)
    #mode2 = np.where(Hurst[bars:]<threshold,0,1)
    if showPlot:
        mode = np.where(SmoothHurst[bars:]<threshold,0,1)
        p=p[bars:]
        nrows=p.shape[0]
...
if __name__ == "__main__":
    ...
    bars=20
    threshold=.6
    smoothHurst(p,bars,threshold=threshold, showPlot=True)